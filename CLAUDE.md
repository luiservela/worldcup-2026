...
it also has Matches, Groups, Teams, Players and Stadiums pages with detail views.
...
- **Win-probability charts (Plotly).** Hero (`renderForecast`) = an **hourly stacked
  ...
  `#fc-crests`, positioned off Plotly's `_fullLayout._size`). Bar/fill colour modes
  cycle Vibrant → Team → Complementary → **Tartan (crest)** → **Tartan (flag)**
  (`barMarker`/`tartanMarker`; tartan = a two-colour `fillpattern` weave whose *shape*
  also varies per team — `TARTAN_SHAPES`). ⚙️ Options is a floating dropdown with a
  **"Show matches"** toggle (dims tartans, drops a black band-height line + the
  opponent's crest at each played match of the shown teams, from `event-history.js`)
  and crest-size sliders; 📤 **Share** exports a 1200×630 PNG via `Plotly.toImage`
  (download fallback if `navigator.share` is unavailable). Team pages have a **"Road to
  the title"** line+markers chart (`renderTeamTitleChart`): a Pre reference point +
  that team's matches (group fixtures known up front show as flag ticks).
...
| `event-history.js` / `.json` | `EVENT_HISTORY` `{updated,slots:[{t,utc,phase,matches:[{hc,ac,hs,as}],p:{code}}]}` | win-probs sampled at each match's final whistle → team road-line + hero "Show matches" | `scripts/event-history.mjs` (Polymarket + schedule.js) |
| `flag-colors.js` / `.json` | `FLAG_COLOR` `{code:[{h,s,l},{h,s,l}]}` | each flag's two main colours → "Tartan (flag)" mode | `scripts/flag-colors.py` (Pillow, flagcdn — one-off) |
...
- **`results-snapshot.yml`** — cron **hourly** (at :20) → `results-snapshot.mjs`, then
  (best-effort, after the schedule regen) `event-history.mjs` + `odds-hourly.mjs` so the
  win-prob charts stay in lockstep with results; commits all three as `wc26-results-bot`
...
- **Polymarket** (`gamma-api`/`clob.polymarket.com`, CORS-open, no key) — win
  ...
  hourly, `event-history.js` per-match); `clob…/prices-history?fidelity=N` gives the tape.
